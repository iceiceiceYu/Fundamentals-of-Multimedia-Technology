...
set_bits(BITS *bits, INT16 data) {
    ...
    UINT16 normalized_data;
    int i;

    // 如果data为负值，将其转换为正值
    if (data < 0) {
        normalized_data = (UINT16) (~data + 1);
    } else {
        normalized_data = (UINT16) data;
    }

    // 通过位运算，求normalized_data的绝对值的位长度
    for (i = 15; i >= 0; i--) {
        if ((normalized_data & (1 << i)) != 0) {
            break;
        }
    }

    // 将len和val写入bits，并将负值加上2^x映射为正值
    bits->len = (UINT8) (i + 1);
    if (data < 0) {
...
print_help() {
    ...
    printf("SEE Total Tutorial, TRY input:\n");
    printf("    cjpeg {BMP} {JPEG}\n");
    printf("Fast Usage, TRY input:\n");
    printf("    cjpeg -outfile [dest_file.jpeg] [source_file.bmp]\n");
    printf("Author: Yu Zhexuan <18302010018@fudan.edu.cn>\n");
    printf("\n");
}
